...
// 应用展示的url
let appShowUrl = "https://v.bitdj.life:8185/"
// 获取图像资源
let apiServerBase = "https://v.bitdj.life:8186"
...
// let autoPlayAudio = false; // 自动播放音频
    ...
    if (onClickFunction) {
        videoPlayOverlay.addEventListener('click', function onOverlayClick(event) {
            ...
            if ( htmlElement.id === 'playButton' ) {
                ...
                let clickPlayButtonOverlay = document.getElementById('clickPlayButtonOverlay')
                if ( clickPlayButtonOverlay && clickPlayButtonOverlay.style.display !== 'none'){
                    clickPlayButtonOverlay.style.display = 'none'
                }
            }
            // 事件触发后移除监听器，避免重复触发
                ...
                if (value === 100){
                    ...
                    setTimeout(() => {
                        ...
                        if (loadingCoverHtml!==null){
                            ...
                            // loadingCoverHtml.style.display = "block" // 隐藏
                        }
                    }, 700)
...
// 自己在设置一个遮罩, 用来执行刷新连接
function showClickPlayTextOverlay() {
    console.log('是否设置了我?')
    let imgMy = document.createElement('img');
    imgMy.id = 'playButtonMy';
    imgMy.src = 'images/play-bai-512.png';
    imgMy.alt = 'Start Streaming My';
    // 创建一个div元素, 设置id为'clickPlayTextOverlay', 设置innerHTML为'点击开始'
    let clickPlayButtonOverlay = document.getElementById('clickPlayButtonOverlay')
    if (!clickPlayButtonOverlay){
        let playerDiv = document.getElementById('player');
        clickPlayButtonOverlay = document.createElement('div');
        clickPlayButtonOverlay.id = 'clickPlayButtonOverlay';
        playerDiv.appendChild(clickPlayButtonOverlay);
    }
    while ( clickPlayButtonOverlay.lastChild){
        clickPlayButtonOverlay.removeChild(clickPlayButtonOverlay.lastChild)
    }

    // 将img添加为子元素
    clickPlayButtonOverlay.appendChild(imgMy)

    // 添加点击事件: 当点击时
    clickPlayButtonOverlay.addEventListener('click', function clickEvent(event){
        playStream() // 播放流
        // 同时如果存在videoPlayOverlay的display不是none, 就设置为none
        let videoPlayOverlay = document.getElementById('videoPlayOverlay')
        ...
        clickPlayButtonOverlay.style.display = 'none'
        //移除该事件监听
        clickPlayButtonOverlay.removeEventListener('click', clickEvent)
    })
...
//     let windowAspectRatio = window.innerHeight / window.innerWidth; //窗口的高宽比
//     let playerAspectRatio = playerElement.clientHeight / playerElement.clientWidth; //播放器的高宽比
//     // We want to keep the video ratio correct for the video stream
//     let videoAspectRatio = videoElement.videoHeight / videoElement.videoWidth; //视频的高宽比
...
function resizePlayerStyleToFillWindow(playerElement) {
    ...
    // Fill the player display in window, keeping picture's aspect ratio.
    let windowAspectRatio = window.innerWidth / window.innerHeight ; //窗口的高宽比
    let playerAspectRatio = playerElement.clientHeight / playerElement.clientWidth; //播放器的高宽比
    // We want to keep the video ratio correct for the video stream
    let videoAspectRatio = videoElement.videoHeight / videoElement.videoWidth; //视频的高宽比
    if (isNaN(videoAspectRatio)) {
        ...
        // console.log(`playerUI的宽度和高度为:`, playerUI.clientWidth, playerUI.clientHeight)
        // styleWidth = playerUI.clientWidth;
        // styleHeight = playerUI.clientHeight;
        styleWidth = window.innerHeight;
        styleHeight = window.innerWidth;
        console.log('styleWidth:', styleWidth, 'styleHeight:', styleHeight)
        console.log(`playEl`)
        styleTop = 0;
        styleLeft = 0;
        playerElement.style = "top: " + styleTop + "px; left: " + styleLeft + "px; width: " + styleWidth + "px; height: " + styleHeight + "px; cursor: " + styleCursor + "; " + styleAdditional;

        console.log(`1设置播放器尺寸为窗口大小为:`, "top: " + styleTop + "px; left: " + styleLeft + "px; width: " + styleWidth + "px; height: " + styleHeight + "px; cursor: " + styleCursor + "; " + styleAdditional);



    } else if (windowAspectRatio < playerAspectRatio) { // 屏幕的高小了,要以高为准
        // 窗口高度是限制因素，因此要保持长宽比适当改变宽度
        styleWidth = Math.floor(window.innerWidth / videoAspectRatio);
        styleHeight = window.innerWidth;
        styleTop = 0;
        styleLeft = Math.floor((window.innerHeight - styleWidth) * 0.5);
        //Video is now 100% of the playerElement, so set the playerElement style
        playerElement.style = "top: " + styleTop + "px; left: " + styleLeft + "px; width: " + styleWidth + "px; height: " + styleHeight + "px; cursor: " + styleCursor + "; " + styleAdditional;
        console.log(`2设置播放器尺寸为窗口大小为:`, "top: " + styleTop + "px; left: " + styleLeft + "px; width: " + styleWidth + "px; height: " + styleHeight + "px; cursor: " + styleCursor + "; " + styleAdditional);
    } else {
        // Window width is the constraining factor so to keep aspect ratio change height appropriately
        styleWidth = window.innerHeight;
        styleHeight = Math.floor(window.innerHeight * videoAspectRatio);
        styleTop = Math.floor((window.innerWidth - styleHeight) * 0.5);
        styleLeft = 0;
        //Video is now 100% of the playerElement, so set the playerElement style
        playerElement.style = "top: " + styleTop + "px; left: " + styleLeft + "px; width: " + styleWidth + "px; height: " + styleHeight + "px; cursor: " + styleCursor + "; " + styleAdditional;
        console.log(`3设置播放器尺寸为窗口大小为:`, "top: " + styleTop + "px; left: " + styleLeft + "px; width: " + styleWidth + "px; height: " + styleHeight + "px; cursor: " + styleCursor + "; " + styleAdditional);
    }
}
// 将播放器尺寸调整为实际大小
function resizePlayerStyleToActualSize(playerElement) {
    ...
    if (videoElement.length > 0) {
        ...
        let Top = Math.floor((window.innerWidth - styleHeight) * 0.5);
        let Left = Math.floor((window.innerHeight - styleWidth) * 0.5);
...
function resizePlayerStyle(event) {
    ...
    let checkBox = document.getElementById('enlarge-display-to-fill-window-tgl');
    // 判断窗口尺寸是否小于播放器视频尺寸
    let windowSmallerThanPlayer = window.innerHeight < playerElement.videoWidth || window.innerWidth < playerElement.videoHeight;
    // 检查复选框是否不为null（存在）
    if (checkBox !== null) {
        ...
        if (checkBox.checked || windowSmallerThanPlayer) {
            ...
            resizePlayerStyleToFillWindow(playerElement); // 手机版默认就是填满窗口
        }
...
function registerInputs(playerElement) {
    ...
    registerMouseEnterAndLeaveEvents(playerElement); // 注册鼠标进入和离开事件
    registerTouchEvents(playerElement); // 注册触摸事件
}
...
function registerTouchEvents(playerElement) {
    // 我们需要给每个手指分配一个唯一的标识符
    // 我们通过将每个Touch对象映射到标识符来做到这一点
    let fingers = [9, 8, 7, 6, 5, 4, 3, 2, 1, 0]; // 初始化一个包含手指标识符的数组
    let fingerIds = {}; // 用于存储每个触摸对象的标识符

    function rememberTouch(touch) {
        // 从数组中弹出一个标识符并分配给手指
        let finger = fingers.pop(); // 弹出数组中的最后一个元素作为标识符
        if (finger === undefined) { // 如果没有可用的标识符打印错误信息
            console.log('没有可用的触摸标识符');
        }
        fingerIds[touch.identifier] = finger; // 将标识符分配给当前触摸对象
    }

    function forgetTouch(touch) {
        // 将标识符重新放回数组中
        fingers.push(fingerIds[touch.identifier]); // 将标识符放回数组
        // 将数组重新排序为降序，这样确保最小的标识符将最先弹出
        fingers.sort(function(a, b){return b - a}); // 将数组按降序排序
        delete fingerIds[touch.identifier]; // 删除该触摸对象的标识符
    }
    // 示例用法: 将旋转后的坐标映射回蓝色矩形的坐标系,解决旋转问题
    function mapYellowToBlue(x, y, w, h) {
        // 第一步：逆时针旋转90度
        let rotatedX = y;
        let rotatedY = h - x;

        // 第二步：平移变换，将点移动回蓝色矩形的坐标系
        let translatedX = rotatedX;
        let translatedY = rotatedY;
...
//     let w = 200; // 矩形的宽度
//     let h = 100; // 矩形的高度
//     let pointYellow = { x: 0, y: 30 }; // 黄色矩形中的点
//     let pointBlue = mapYellowToBlue(pointYellow.x, pointYellow.y, w, h);
//
//     console.log(`在蓝色矩形中的映射点: (${pointBlue.x}, ${pointBlue.y})`);

    function emitTouchData(type, touches) {
        // 遍历所有触摸点
        for (let t = 0; t < touches.length; t++) { // 循环遍历所有触摸点
            let numTouches = 1; // 发送此消息的触摸数
            let touch = touches[t]; // 获取当前触摸点
            // 计算触摸相对于playerElement的坐标
            // let x = touch.clientX - playerElement.offsetLeft; // 计算触摸点的X坐标
            // let y = touch.clientY - playerElement.offsetTop; // 计算触摸点的Y坐标
            let x = touch.clientX - playerElement.offsetLeft; // 计算触摸点的X坐标
            let y = touch.clientY - playerElement.offsetTop; // 计算触摸点的Y坐标
            // console.log(`初始化坐标: (${x}, ${y})`);
            let mapCoord = mapYellowToBlue(x, y, playerElement.clientWidth, playerElement.clientHeight);
            x = mapCoord.x;
            y = mapCoord.y;
            // console.log(`映射后的坐标: (${x}, ${y})`)
    ...
    if (inputOptions.fakeMouseWithTouches) {
        ...
        playerElement.ontouchstart = function(e) {
            // 为每个触摸分配唯一标识符
            for (let t = 0; t < e.changedTouches.length; t++) { // 循环遍历所有触摸点
                rememberTouch(e.changedTouches[t]); // 为当前触摸点分配标识符
            }

            if (print_inputs) { // 如果需要打印输入
                console.log('touch start'); // 打印触摸开始信息
            }
            emitTouchData("TouchStart", e.changedTouches); // 发送触摸开始事件数据
            e.preventDefault();
        };

        playerElement.ontouchend = function(e) {
            if (print_inputs) { // 如果需要打印输入
                console.log('touch end'); // 打印触摸结束信息
            }
            emitTouchData("TouchEnd", e.changedTouches); // 发送触摸结束事件数据

            // 回收之前分配给每个触摸的唯一标识符
            for (let t = 0; t < e.changedTouches.length; t++) { // 循环遍历所有触摸点
                forgetTouch(e.changedTouches[t]); // 回收当前触摸点的标识符
            }
        ...
        playerElement.ontouchmove = function(e) {
            if (print_inputs) { // 如果需要打印输入
                console.log('touch move'); // 打印触摸移动信息
            }
            emitTouchData("TouchMove", e.touches); // 发送触摸移动事件数据
            e.preventDefault(); // 阻止默认事件
        };
    }
}
